...
String app_port = "";
String app_path="";
String web_path="/usr/local/weblogic/apache";
String app_console="/console";
String console_protocol="http";
String con_user="admin";
String con_pwd="admin";
		...
		//proj_code = rs2.getInt("proj_code");
		 ...
		 if (!"".equals(appSoft) && appSoft != null) {
				...
				if (appSoftLow.indexOf("weblogic") == 0) {
					...
					app_port = "9043";
					app_path = "/usr/local/was/IBM/WebSphere/AppServer/profiles/"+rs.getString("new_env").toLowerCase();
					web_path = "/usr/local/was/IBM/HTTPServer";
...
<div align="center"><font color="#3333cc"><strong><br>
  <span class="s105"><%=rs.getString("new_env")%> 环境表提交</span></strong></font>
</div>
...
<table width="80%" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E" bordercolordark="#EFEFEF" align="center">
  ...
  <tr> 
    <td width="37%" bgcolor="#D2D2D2" class="STYLE1 en"><strong>Env IP</strong></td>
    <td class="en" width="63%"><input type="text" name="ENV_IP" required pattern="((^|\.)((25[0-5])|(2[0-4]\d)|(1\d\d)|([1-9]?\d))){4}$" value="<%=rs.getString("env_ip")%>" class="en" size="16"></td>
  </tr>
  <tr> 
    <td width="37%" bgcolor="#D2D2D2" class="STYLE1 en"><strong>MAC IP</strong></td>
    <td class="en" width="63%"> <input type="text" name="MAC_IP" required pattern="((^|\.)((25[0-5])|(2[0-4]\d)|(1\d\d)|([1-9]?\d))){4}$"  value="<%=rs.getString("mac_ip")%>" class="en" size="16"></td>
  </tr>
  <tr> 
    <td width="37%" bgcolor="#D2D2D2" class="STYLE1 en"><strong>webServer实例路径</strong></td>
    <td class="en" width="63%"><input type="text" name="WEB_INST_PATH" value="<%=web_path%>" class="en"   size="65"></td>
  </tr>
  <tr> 
    <td width="37%" bgcolor="#D2D2D2" class="STYLE1 en"><strong>HTML路径</strong></td>
    <td class="en" width="63%"><input type="text" name="HTML_PATH" value="NA" class="en"  size="35"></td>
  </tr>
  <tr> 
    <td width="37%" bgcolor="#D2D2D2" class="STYLE1 en"><strong>appServer实例路径</strong></td>
    <td class="en" width="63%"><input type="text" name="APP_INST_PATH" value="<%=app_path%>" class="en"  size="65"></td>
  </tr>
  <tr> 
  ...
  <tr> 
    ...
    <td class="en" width="63%"><input type="url" required="required" name="APP_CONSOLE" value="<%=console_protocol%>://<%=rs.getString("env_ip")%>:<%=app_port%>/<%=app_console%>" class="en" size="65"></td>
  </tr>
  	<%
	...
	%>
  <tr> 
    <td width="37%" bgcolor="#D2D2D2" class="STYLE1 en"><strong>SSH&FTP用户</strong></td>
    <td class="en" width="63%"><input type="text" name="ENV_USR" value="<%=app_user%>" class="en" size="35"></td>
  </tr>
  <tr> 
    <td width="37%" bgcolor="#D2D2D2" class="STYLE1 en"><strong>SSH&FTP密码</strong></td>
  ...
  <tr> 
    ...
    <td class="en" width="63%"><input type="url" required="required" name="PROJ_DNS" value="http://<%=rs.getString("env_ip")%>/insurance/mainMenu.do" class="en" size="35"></td>
  </tr>
    ...
    <td class="en" width="63%">
        ...
        <%
        String sql_temp="select * from ts_print a, ts_proj b where a.proj_id=b.proj_id(+)";
        ResultSet rs_temp=db.execSQL(sql_temp);
	...
	<a href="ts_print_query.jsp">Print Server List</a>
    </td>
  </tr>
  <tr> 
    <td width="37%" bgcolor="#D2D2D2" class="STYLE1 en"><strong>打印目录</strong></td>
    <td class="en" width="63%"><input type="text" name="print_dir" class="en" size="35" value="d:\<%=app_user%>"></td>
  </tr>
  ...
  <tr> 
    ...
    <td width="78%" class="en">                  
	<textarea name="startup_script" cols="60"  rows="2">sh <%=app_path%>/restart.sh</textarea>
	</td>
  </tr>
  <tr> 
