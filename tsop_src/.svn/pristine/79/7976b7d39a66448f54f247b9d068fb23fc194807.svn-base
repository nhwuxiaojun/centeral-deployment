...
<html>
<head>
<title>TS DB Environment Query</title>
...
<tr><td>&nbsp;</td></tr>
<form id="form1" name="form1" action="ts_db_env_query.jsp" method="get" class="en">
<input name="flag" value="y" type=hidden>
<tr>
<td align="left" class="en" width="20%">
        &nbsp;<strong><font color="#006699" class="MenuBarItemSubmenu">DB Environment Query</font></strong> &nbsp;&nbsp;
...
<%
	...
	<%}%>
         ...
         <select name="col_name1" id="col_name1">
              <option value="proj_env" <%if("proj_env".equals(request.getParameter("col_name1"))) { out.print("selected");}%>>Environment Name</option>
              <option value="db_user" <%if("db_user".equals(request.getParameter("col_name1"))) { out.print("selected");}%>>DB User Name</option>
...
// Start Search by project_name, group, code, etc.                
           if("".equals(request.getParameter("col_value1"))||request.getParameter("col_value1")==null){%>
                <input type="text" id="search" name="col_value1" value="" size="30" placeholder="Environment IP or Name">&nbsp;
            <%}else{%>
...
common_sql="select a.ENV_IP,a.APP_INST_PATH,c.db_id ENV_ID,c.proj_env env_name,a.app_url,c.db_version app_version,a.ENV_USR,a.ENV_PWD,a.APP_PORT,b.LDAP_NAME,c.DB_USER,c.DB_USER_PASSWD,f.account from ts_env_app a, ts_db b, ts_env_db c, ts_env d, ts_proj e, ts_member f where c.db_ldap_name=b.LDAP_NAME(+) and c.env_id=a.env_id(+) and c.env_id=d.env_id(+) and d.proj_id=e.proj_id(+) and lower(c.DB_USER_TYPE)='owner' and e.owner1=f.no(+)";
common_sql2="select a.ENV_IP,a.APP_INST_PATH,c.db_id ENV_ID,c.proj_env env_name,a.app_url,c.db_version app_version,a.ENV_USR,a.ENV_PWD,a.APP_PORT,b.LDAP_NAME,c.DB_USER,c.DB_USER_PASSWD,f.account from ts_env_app a, ts_db b, ts_env_db c, ts_env d, ts_proj e, ts_member f, ts_recent_env g  where c.db_ldap_name=b.LDAP_NAME(+) and c.env_id=a.env_id(+) and c.env_id=d.env_id(+) and d.proj_id=e.proj_id(+) and lower(c.DB_USER_TYPE)='owner' and e.owner1=f.no(+)";
...
if ("".equals(col_value1)||(col_value1==null)){
	if (tmpproj_name==null||"".equals(tmpproj_name)) {
		// default access
                sql="select * from ("+common_sql2+" and env_db_status='running' and g.env_id=d.env_id and g.user_name='"+session.getAttribute("username")+"'order by g.last_access desc ) where  rownum<20";
		System.out.println("---sql1---");
	} else {
		// search by project name
		sql=common_sql+" and env_db_status='running' and d.proj_id like '"+tmpproj_name+"' order by ENV_NAME";
	...
	//if (1==2) {
             sql=common_sql+" and env_db_status='running' and rownum<10 order by ENV_NAME";
		System.out.println("---sql3---");
		...
		if ("ext_env_ip".equals(col_name1)) {
			...
			sql=common_sql+" and env_db_status='running' and upper("+ col_name1.toString()+") like '%"+ col_value1.toString().trim().toUpperCase() + "%' order by env_name";
		}
...
<%while(rs2.next()){%>
	<%if("0".equals(rs2.getString("valid"))){%>
		<tr><td>&nbsp;</td></tr>
			<tr>
			<td align="center">
			<form name="form3" action="ts_db_env_query.jsp">
...
<thead>
	<tr bgcolor="#006699" height="24">
	  ...
	  <td align="center" class="en" width="10%"><font color="#ffffff"><b>DB User</b></font></td>
	  ...
	  <td align="center" class="en" width="6%"><font color="#ffffff"><b>App Restart</b><font color="red"></font></font></td>
...
<tbody>
	<%while(rs.next()){
		...
		%>
	  <td align="center" class="en"><a href="#" onClick="open_win('add_favorite.jsp?id=<%=rs.getInt("ENV_ID")%>',300,200)"><img src='../images/favorite1.gif' alt='Add to my favorites' border=0></a></td>
...
//	out.println("<a href="+ext_url+" target=_blank>"+rs4.getString("EXTERNAL_IP")+":"+rs4.getString("EXTERNAL_PORT")+"</a>");
	out.println("<a href="+int_url+" target=_blank>Internal</a>|<a href="+ext_url+" target=_blank>External</a>");
}
if (sum4==0) {
...
%> 
	...
	<td align="left"><font color=blue><%=rs.getString("DB_USER")%></font></td>
...
%>
<a href="#"  onClick="open_win('../ts_db_deploy_env.jsp?env_id=<%=rs.getInt("ENV_ID")%>&env_name=<%=rs.getString("ENV_NAME")%>',800,550);"><font color=red><b>Deploy</b></font></a>
<%
}
} catch (Exception e) {
	e.printStackTrace();
