    ...
    if ("y".equals(request.getParameter("flag"))) {
        if ("".equals(request.getParameter("txt_new_env"))) {
            if ("".equals(request.getParameter("txt_submit_datetime"))) {
                sql = "select * from a_supply where  apply_type is null  order by id desc";
            } else {
                sql = "select * from a_supply where  apply_type is null and sysdate - apply_time <= " + request.getParameter("txt_submit_datetime") + " order by id desc";
            }
        } else {
            if ("".equals(request.getParameter("txt_submit_datetime"))) {
                sql = "select * from a_supply where  apply_type is null and  upper(new_env) like '%" + Tools.toGB(request.getParameter("txt_new_env").toString().trim()).toUpperCase() + "%' order by id desc";
            } else {
                sql = "select * from a_supply where  apply_type is null  and upper(new_env) like '%" + Tools.toGB(request.getParameter("txt_new_env").toString().trim()).toUpperCase() + "%' and sysdate - apply_time <= " + request.getParameter("txt_submit_datetime") + " order by id desc";
            }
        }
    } else {
        sql = "select * from a_supply where  apply_type is null and rownum<11 and (env_status='DOING' or db_status='DOING') order by id desc";
    }
...
<form name="form1" action="" method="post">
    ...
    <table align="center" cellpadding="0" cellspacing="0" border="0" width="95%">
        <tr>
            ...
            %>&nbsp;&nbsp;<input type="button" name="button2" value="新增" class="button1" onClick="add()"><%
                    }
                }
            %>
            </td>
...
<hr>
<table width="98%" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E" bordercolordark="#EFEFEF"
       align="center">
    <tr bgcolor="#D2D2D2">
    ...
    <tr bgcolor="#FCFCFC">
        ...
        <td align="center">&nbsp;<a href="project_apply_detail.jsp?id=<%=rs.getInt("id")%>" target="_blank"><font
                color="#3333cc">详细</font></a>&nbsp;</td>
        <%sql_user = "select * from a_user where usr = '" + session.getAttribute("username") + "'";%>
        <%rs_user = db.execSQL(sql_user);%>
        <%while (rs_user.next()) {%>
        <%if ("2".equals(rs_user.getString("valid"))) {%>
        <td align="center">&nbsp;<a href="project_apply_update.jsp?id=<%=rs.getInt("id")%>"><font
                color="#3333cc">修改</font></a>&nbsp;</td>
        <td align="center">&nbsp;<a href="project_apply_del.jsp?id=<%=rs.getInt("id")%>"><font color="#3333cc">删除</font></a>&nbsp;
        </td>
        <%}%>
        <%}%>
    </tr>
