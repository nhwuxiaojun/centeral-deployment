...
function add(){
	window.location="only_db_reply_add.jsp";
}
</script>
...
if("y".equals(request.getParameter("flag"))){
	if("".equals(request.getParameter("txt_new_env"))){
		if("".equals(request.getParameter("txt_submit_datetime"))){
			sql = "select * from a_supply where   apply_type='DB' order by id desc";
		}else{
			sql = "select * from a_supply where   apply_type='DB'  and sysdate - apply_time <= " + request.getParameter("txt_submit_datetime") + " order by id desc";
		}
	}else{
		if("".equals(request.getParameter("txt_submit_datetime"))){
			sql = "select * from a_supply where   apply_type='DB' and upper(new_env) like '%" + Tools.toGB(request.getParameter("txt_new_env").toString().trim()).toUpperCase() + "%' order by id desc";
		}else{
			sql = "select * from a_supply where   apply_type='DB' and  upper(new_env) like '%" + Tools.toGB(request.getParameter("txt_new_env").toString().trim()).toUpperCase() + "%' and sysdate - apply_time <= " + request.getParameter("txt_submit_datetime") + " order by id desc";
		}
	}
}else{
	sql = "select * from a_supply where   apply_type='DB'  and rownum<11 and  db_status='DOING' order by id desc";
}
System.out.print(sql);
rs = db.execSQL(sql);
%>
<div align="center"><font color="#3333cc"><strong><br>
  <span class="s105">单数据库环境答复列表</span></strong></font>
</div>
...
<table width="98%" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E" bordercolordark="#EFEFEF" align="center">
	...
	<tr bgcolor="#FCFCFC">
	  ...
	  <td align="center" >&nbsp;<a href="project_apply_detail.jsp?id=<%=rs.getInt("id")%>&flag=2" target="_blank"><font color="#3333cc">详细</font></a>&nbsp;</td>
	  <%sql_user = "select * from a_user where usr = '" + session.getAttribute("username") + "'";%>
	  <%rs_user = db.execSQL(sql_user);%>
	  <%while(rs_user.next()){%>
	  	<%if("2".equals(rs_user.getString("valid"))){%>
	  		<td align="center">&nbsp;<a href="only_app_db_apply_del.jsp?id=<%=rs.getInt("id")%>&flag=2"><font color="#3333cc">删除</font></a>&nbsp;</td>
	  	<%}%>
	  <%}%>
    </tr>
	<%}%>
