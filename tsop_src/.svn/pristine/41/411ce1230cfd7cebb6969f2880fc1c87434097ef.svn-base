...
<%
	sql="select * from a_env_name order by proj_grp,proj_name";
	rs=db.execSQL(sql);
	%>
...
<div align="center"><font color="#3333cc"><strong><br>
  <span class="s105">单数据库环境申请（<font color="#FF0000">*</font>项为必填）</span></strong></font>
</div>

<form name="form1" action="only_db_build_add_finish.jsp" method="post" class="en">
...
<table width="95%" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E" bordercolordark="#EFEFEF" align="center">
  ...
  <tr> 
	...
	<select name="db_creator" class="select5">
        <option value="<%=session.getAttribute("username")%>"><%=session.getAttribute("username")%></option>^M
	<%while(rs.next()){%>
