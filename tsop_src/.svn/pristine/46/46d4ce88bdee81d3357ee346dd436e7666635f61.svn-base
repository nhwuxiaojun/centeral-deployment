...
<html>
<head>
    <title>TS Environment Query</title>
            ...
            <tr>
                <td align="left" class="en" width="20%">
                    ...
                    <select name="proj_name"
                        ...
                        <option
                                <%
                                    if (rs3.getString("proj_name").equals(request.getParameter("proj_name"))) {
                                        out.print("selected=selected ");
                                    }
                                %>
                                value="<%=rs3.getString("proj_name")%>"><%=rs3.getString("proj_name")%>
                        </option>
                        <%}%>
                    </select>
                    &nbsp;
                    <select name="col_name1" id="col_name1">
                        ...
                        <option value="ext_env_ip" <%
                            if ("ext_env_ip".equals(request.getParameter("col_name1"))) {
                                out.print("selected");
                            }
                        %>>External Env IP
                        </option>
                        <option value="app_version" <%
                            if ("app_version".equals(request.getParameter("col_name1"))) {
                                out.print("selected");
                            }
                        %>>Current Version
                        </option>
...
// End Search by project_name, group, code, etc.   
                    ...
                    <%
                        ...
                        String common_sql2 = new String();
//common_sql="select * from ts_env_app a, ts_db b, ts_env_db c, ts_env d, ts_proj e, ts_member f where c.db_ldap_name=b.LDAP_NAME(+) and a.env_id=c.env_id(+) and a.env_id=d.env_id(+) and d.proj_id=e.proj_id(+) and e.owner1=f.no(+)";
//common_sql="select * from ts_env_app a, ts_db b, ts_env_db c, ts_env d, ts_proj e, ts_member f where a.db_ldap_name=b.LDAP_NAME(+) and a.env_id=c.env_id(+) and a.env_id=d.env_id(+) and d.proj_id=e.proj_id(+) and e.owner1=f.no(+)";
                        common_sql = "select a.ENV_IP,a.APP_INST_PATH,a.ENV_ID,d.env_name,a.app_url,A.app_version,a.ENV_USR,a.ENV_PWD,a.APP_PORT,b.LDAP_NAME,c.DB_USER,c.DB_USER_PASSWD,f.account from ts_env_app a, ts_db b, ts_env_db c, ts_env d, ts_proj e, ts_member f where c.db_ldap_name=b.LDAP_NAME(+) and a.env_id=c.env_id(+) and a.env_id=d.env_id(+) and d.proj_name=e.proj_name(+) and d.proj_id=e.proj_id(+) and e.owner1=f.no(+)";
                        common_sql2 = "select a.ENV_IP,a.APP_INST_PATH,a.ENV_ID,d.env_name,a.app_url,A.app_version,a.ENV_USR,a.ENV_PWD,a.APP_PORT,b.LDAP_NAME,c.DB_USER,c.DB_USER_PASSWD,f.account from ts_env_app a, ts_db b, ts_env_db c, ts_env d, ts_proj e, ts_member f, ts_recent_env g  where c.db_ldap_name=b.LDAP_NAME(+) and a.env_id=c.env_id(+) and a.env_id=d.env_id(+) and d.proj_name=e.proj_name(+) and d.proj_id=e.proj_id(+) and e.owner1=f.no(+)";


                        if ("".equals(col_value1) || (col_value1 == null)) {
                            if (tmpproj_name == null || "".equals(tmpproj_name)) {
                                // default access
                                //	sql=common_sql+" and env_state='running' and rownum<1 order by a.create_date desc";
                                //      sql="select * from ("+common_sql+" and env_state='running' order by a.create_date desc ) where  rownum<1";
                                sql = "select * from (" + common_sql2 + " and env_state='running' and g.env_id=d.env_id and g.user_name='" + session.getAttribute("username") + "'order by g.last_access desc ) where  rownum<20";
                                System.out.println("---sql1---");
                            } else {
                                // search by project name
                                sql = common_sql + " and env_state='running' and d.proj_name like '" + tmpproj_name + "' order by ENV_NAME";
    ...
    <form name="form2">
        ...
        <table id="maintable" width="98%" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E"
            ...
            <tr bgcolor="#3333cc" height="24">
                ...
                <td align="center" class="en" width="8%"><font color="#ffffff"><b>Deploy</b></font></td>
                <td align="center" class="en" width="10%"><font color="#ffffff"><b>App Access</b></font></td>
                <td align="center" class="en" width="8%"><font color="#ffffff"><b>App Restart</b><font
            ...
            <td align="center" class="en"><a href="#"
                                             onClick="open_win('add_favorite.jsp?id=<%=rs.getInt("ENV_ID")%>',300,200)"><img
                    src='../images/favorite1.gif' alt='Add to my favorites' border=0></a></td>
            ...
            <td align="left" class="en"><a href="#"
                                           onClick="open_win('env_info_detail.jsp?id=<%=rs.getInt("ENV_ID")%>&env_name=<%=rs.getString("ENV_NAME")%>',850,640);"><%=rs.getString("ENV_NAME")%>
            </a></font></td>
            <td align="center">
                <%
...
//		ext_url=int_url.replace(rs4.getString("INTERNAL_IP").trim(),rs4.getString("EXTERNAL_IP"));
//		ext_url=ext_url.replace(rs4.getString("INTERNAL_PORT").trim(),rs4.getString("EXTERNAL_PORT"));
...
//	out.println("<a href="+ext_url+" target=_blank>"+rs4.getString("EXTERNAL_IP")+":"+rs4.getString("EXTERNAL_PORT")+"</a>");
                        out.println("<a href=" + int_url + " target=_blank>Int</a>");
                        out.print("|<a href=" + ext_url + " target=_blank>Ext</a>");
                    }
                    if (sum4 == 0) {
            ...
            <td align="left" class="en">
                [<a href="#"
                    onClick="open_win('../ts_deploy_history.jsp?env_id=<%=rs.getInt("ENV_ID")%>&env_name=<%=rs.getString("ENV_NAME")%>',1000,600)">History</a>]
                <a href="/tsop/realtime_show_log.jsp?env_id=<%=rs.getInt("ENV_ID")%>&env_name=<%=rs.getString("ENV_NAME")%>&filename=<%=rs.getString("APP_VERSION")%>"
                   target="_blank"><%=rs.getString("APP_VERSION")%>
                </a>
            </td>
            </td>
...
//System.out.println("sql4---"+sql4);
                ...
                %>
                <a href="#"
                   onClick="open_win('../ts_deploy_env.jsp?env_id=<%=rs.getInt("ENV_ID")%>&env_name=<%=rs.getString("ENV_NAME")%>',800,550);"><font
                ...
                %>
                <a href="javascript:"
                   onClick="execSamba('\\\\172.17.1.7\\Application\\deploy\\<%=rs.getString("ENV_NAME")%>')">[LOG]</a>
            </td>
            </td>

            <td align="center" class="en">
                <a href="javascript:"
                   onClick="execPutty('<%=rs.getString("ENV_IP")%>','<%=rs.getString("ENV_USR")%>','<%=rs.getString("ENV_PWD")%>')">SSH</a>&nbsp;|
                <a href="javascript:"
                ...
                <a href="javascript:" onClick="execSamba('\\\\<%=rs.getString("ENV_IP")%>')">SMB</a>
            </td>
            <td align="center" class="en" width="6%">
                <a href="javascript:"
                   onClick="restart_app('<%=rs.getString("ENV_IP")%>','<%=rs.getString("ENV_USR")%>','<%=rs.getString("ENV_PWD")%>')">App</a>&nbsp;|
                <a href="javascript:"
                   onClick="restart_batch('<%=rs.getString("ENV_IP")%>','<%=rs.getString("ENV_USR")%>','<%=rs.getString("ENV_PWD")%>')">Batch</a>
            </td>
            <td align="center">
                <%
                    try {
...
//		 System.out.println("Host "+rs.getString("ENV_IP")+" unreachable.\n");
            ...
            <td align="center" class="en"><a href="javascript:"
                                             onClick="execPLSQL('<%=rs.getString("LDAP_NAME")%>','<%=rs.getString("DB_USER")%>','<%=rs.getString("DB_USER_PASSWD")%>')">owner</font></a>
                |&nbsp;<a href="javascript:"
                          onClick="execPLSQL('<%=rs.getString("LDAP_NAME")%>','<%=rs.getString("DB_USER")+"_read"%>','<%=rs.getString("DB_USER")+"_read_only"%>')">read</font></a>
                |&nbsp;<a href="#"
                          onclick="open_win('../extract_awr.jsp?env_name=<%=rs.getString("ENV_NAME")%>&db_sid=<%=rs.getString("LDAP_NAME")%>&db_ldap_name=<%=rs.getString("LDAP_NAME")%>',420,460);"><font
                        color=blue>AWR</font></a>
            </td>
            <td align="center" class="en" width="10%"><a
                    href="mailto:<%=rs.getString("account")%>@ebaotech.com"><%=rs.getString("account")%>
            </a></td>
            </tr>
            <%}%>
            </tbody>
...
<br>
<small>
    &nbsp;&nbsp;Notice: If you want to use SSH, you need to download and install Putty from <a
