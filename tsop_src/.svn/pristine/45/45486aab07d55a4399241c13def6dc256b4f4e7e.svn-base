...
try
{
	...
	{
                //Get record count
                $result = mysql_query("SELECT COUNT(*) AS RecordCount FROM ts_ip_map_matrix;");
                $row = mysql_fetch_array($result);
                $recordCount = $row['RecordCount'];

		//Get records from database
		$result = mysql_query("SELECT * FROM ts_ip_map_matrix ORDER BY " . $_GET["jtSorting"] . " LIMIT " . $_GET["jtStartIndex"] . "," . $_GET["jtPageSize"] . ";");
