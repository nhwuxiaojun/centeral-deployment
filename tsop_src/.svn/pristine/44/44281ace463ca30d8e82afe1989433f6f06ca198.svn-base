...
sql="insert into ts_recent_env(env_id,user_name) select "+request.getParameter("id")+",'"+session.getAttribute("username")+"' from dual where not exists ( select * from ts_recent_env where user_name='"+session.getAttribute("username")+"' and env_id="+request.getParameter("id")+")";
...
sql="update ts_recent_env set last_access=sysdate where user_name='"+session.getAttribute("username")+"' and env_id="+request.getParameter("id");
System.out.println(sql);
try {
        i = db.execUpdate(sql);
} catch (Exception e) {
...
sql="select a.env_ip,a.WEB_INST_PATH,a.HTML_PATH,a.app_type,a.APP_INST_PATH,a.APP_PORT,a.inc_deploy_para,a.full_deploy_para,c.db_ldap_name,c.DB_USER,c.DB_USER_PASSWD,a.ENV_USR,a.ENV_PWD,e.proj_grp,a.app_url,a.CREATER,a.create_date,a.PROPOSER,a.ENV_STATE,d.proj_id,e.proj_name,a.DMS_TYPE,a.PRINT_TYPE,a.print_ip,d.env_name,f.print_user,f.print_pwd,a.print_dir  from ts_env_app a, ts_db b, ts_env_db c, ts_env d, ts_proj e, ts_print f where a.db_ldap_name = b.LDAP_NAME(+)   and d.env_id = c.db_id(+)   and d.env_id = a.env_id(+)   and d.proj_id = e.proj_id(+)  and a.print_ip = f.print_ip(+) and (a.env_id="+request.getParameter("id") +" or d.env_name='"+request.getParameter("env_name")+"') and rownum<2";
System.out.println("-----"+sql+"-----\n");
rs = db.execSQL(sql);
%>
...
<html>
<head>
<title><%=rs.getString("ENV_NAME")%> DB Environment Information</title>
<meta http-equiv="Content-Type" content="text/html; charset=gbk">
<link href="../css/ebao_no_ts.css" rel="stylesheet" type="text/css">
</head>
<body>
<div align="center"><br><h3><%=rs.getString("ENV_NAME")%> DB Environment Information</h3></div>
<table width="800" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E" bordercolordark="#EFEFEF" align="center">
	<tr>
	  <td width="200" bgcolor="#D2D2D2" class="td_title"><font color="#0000FF"><strong>Environment IP</strong></font></td>
	...
	<tr>
	  <td  bgcolor="#D2D2D2" class="td_title"><font color="#0000FF"><strong><red>DB Version</red></strong></font></td>
	...
	<tr>
         <td  bgcolor="#D2D2D2" class="td_title"><font color="#0000FF"><strong><red>DB Size</red></strong></font></td>
          <td  bgcolor="#FCFCFC" class="td_title"><button id="query_btn" onclick="window.location='<%=request.getRequestURI()+"?"+request.getQueryString()%>&act=query';">Query</button><font color="red">
           <%
	...
	<tr>
	  ...
	  <td  bgcolor="#FCFCFC" class="td_title"><font color="#0000FF"><%=rs.getString("DB_USER")%>/******</font></td>
	</tr>
	<tr>
	  <td  bgcolor="#D2D2D2" class="td_title"><font color="#0000FF"><strong>SSH&FTP USER & Password</strong></font></td>
	...
	<tr>
	  <td  bgcolor="#D2D2D2" class="td_title"><font color="#0000FF"><strong>Project Name & Code</strong></font></td>
	  <td  bgcolor="#FCFCFC" class="td_title"><font color="#0000FF"><%=rs.getString("PROJ_NAME")%> - <%=rs.getString("PROJ_ID")%></font></td>
	</tr>

		<tr>
	  <td  bgcolor="#D2D2D2" class="td_title"><font color="#0000FF"><strong>DMS & Print Type</strong></font></td>
	  <td  bgcolor="#FCFCFC" class="td_title"><font color="#0000FF"><%=Tools.msNull(rs.getString("DMS_TYPE"))%></font>/<%=Tools.msNull(rs.getString("PRINT_TYPE"))%></font></td>
	</tr>
