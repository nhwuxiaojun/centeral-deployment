...
<main>
    <%
        ...
        if ("y".equals(request.getParameter("flag"))) {
            ...
            sql = "select * from a_proj_env_svr where  env_state='running'  and  rownum<8   " + proj_code_sql + "  order by mac_ip,proj_env,env_ip";
        }
//out.print(sql+"<br>");
        rs = db.execSQL(sql);
    ...
    <%} else {%>
    <br>
    <center><font color="#FF0000" style="{font-size:9pt;}"><%=Tools.toGB(request.getParameter("info"))%>
    </font></center>
    <%}%>
            ...
            <tr>
                ...
                <td class="en" align="center">
                    <SELECT name="proj_grp"
                            onChange="change(document.form2.proj_grp.options[document.form2.proj_grp.selectedIndex].value)">
                ...
                <td align="center" class="en">
                    <select name="proj_name" class="select7">
                        ...
                        <option value="<%=rs3.getString("proj_code")%>"
                                selected="selected"><%=rs3.getString("proj_name")%>
                        </option>
                        <%}%>
                        <%}%>
                    </SELECT>
                ...
                <td align="center" class="en" width="20%"><input type="button" name="button1" value="Query"
                                                                 onClick="check()"></td>
            </tr>
    ...
    <form name="form1" action="" method="post" class="en">
        <table id="maintable" width="98%" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E"
            ...
            <thead>
            <tr height=25 bgcolor="#0000ee">
                <td align="center" class="en" width="10%"><font color="#ffffff"><strong>MAC IP</strong></font></td>
                <td align="center" class="en" width="10%"><font color="#ffffff"><strong>Environment IP</strong></font>
                </td>
                <td align="center" class="en" width="25%"><font color="#ffffff"><strong>Environment
                    Details</strong></font></td>
                <td align="center" class="en" width="25%"><font color="#ffffff"><strong>Grant DB
                    Privileges</strong></font></td>
                <td align="center" class="en" width="10%"><font color="#ffffff"><strong>Status</strong></font></td>
                <td align="center" class="en" width="12%"><font color="#ffffff"><strong>Select</strong></font><input
                        onclick="if (this.checked) $('input[type=checkbox]').attr('checked',true); else $('input[type=checkbox]').attr('checked',false);"
                        id="selectall" type="checkbox" name="env_id" value=""></td>
            </tr>
            </thead>
            <tbody>
            <% int sum = 0;
            ...
            <td align="center" class="en"><font color="#3333cc">&nbsp;<span
                    id="env_<%=rs.getString("ENV_ID")%>"><%=rs.getString("ENV_IP")%></span>&nbsp;</font></td>
            <td align="center" class="en">&nbsp;<a href="#"
                                                   onClick="open_win('new.env_info_detail.jsp?id=<%=rs.getInt("ENV_ID")%>',800,650);"><font
                    color="#3333cc"><u><span id="proj_<%=rs.getString("ENV_ID")%>"><%=rs.getString("PROJ_ENV")%></span></u></font></a>&nbsp;
            </td>
            <td align="center" class="en">&nbsp;<a
                    href="db_access_privi_do.jsp?link=y&txt_db_user_type=owner&proj_env=<%=rs.getString("PROJ_ENV")%>">Grant</a>&nbsp;
            </td>
            ...
            <td align="center" bgcolor="#FCFCFC" class="en"><input type="checkbox" name="env_id"
                                                                   value="<%=rs.getString("ENV_ID")%>"></td>
            </tr>
            <%} // end of while%>
            </tbody>
        </table>
        <center>
            ...
            <input type="text" name="staff_name" id="staff_name" value="" class="text2" placeholder="jiangtao.chen">&nbsp;&nbsp;
                <%}else{%>
            <input type="text" name="staff_name" id="staff_name" value="<%=request.getParameter("staff_name")%>"
                   class="text2">&nbsp;&nbsp;
                <%}%>
            ...
            <input type="text" name="customer_ip" value="" class="text2"
                   pattern="((^|\.)((25[0-5])|(2[0-4]\d)|(1\d\d)|([1-9]?\d))){4}$" placeholder="x.x.x.x">&nbsp;&nbsp;
                <%}else{%>
...
%>
            ...
            <select name="visit_type">
                <%while (rs_temp.next()) {%>
                <option value="<%=rs_temp.getString("login_way")%>"><%=rs_temp.getString("login_way")%>
                </option>
                <%}%>
            </select>
