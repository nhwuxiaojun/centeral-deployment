...
if(flag==1){
	...
	sql = "insert into a_apply(id,new_env,proj_grp,db_version,env_creator,db_creator,proj_proposer,apply_time,status,proj_code,backup_type,db_size,note,apply_type)values(" + id + ",'" + new_env + "','" + proj_grp + "','" + db_ver +  "','" + db_creator + "','" + db_creator + "','" + proj_proposer + "',to_date('" + datestr + "','yyyy-MM-dd hh24:mi:ss'),'DOING','" + proj_code + "','" + backup_type + "'," + db_size  + ",'"+ note + "','DB')";
	out.println(sql+"<br>");
	try {
	i = db.execUpdate(sql);
	} catch (Exception e) {
