...
for(int i=0;i<=delcount;i++){
	if("".equals(request.getParameter("del_" + i))||request.getParameter("del_" + i)==null){
		...
		sql = "delete from a_login_control where login_ip = '" + temp_dir[0] + "' and login_os_user = '" + temp_dir[1] + "' and login_db_user like '" + temp_dir[2] + "%' and login_db_sid = '" + temp_dir[3] + "'";
		//out.print(sql + "<br>");
		j = db.execUpdate(sql);
	}
}
...
<%
if(j==1){
	response.sendRedirect("db_access_privi_query.jsp?info=Delete Succeed!");
}else{
	response.sendRedirect("db_access_privi_query.jsp?info=Delete Failed!");
}
%>
