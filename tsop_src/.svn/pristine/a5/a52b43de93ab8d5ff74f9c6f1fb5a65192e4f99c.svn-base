...
if(flag==1){
	...
	sql = "insert into a_apply(id,new_env,proj_grp,sys_ver,app_soft,env_creator,proj_proposer,apply_time,status,proj_code,backup_type,note,apply_type)values(" + id + ",upper('" + new_env + "'),'" + proj_grp + "','" + sys_ver + "','" + appsoft_ver +  "','" + env_creator +  "','" + proj_proposer + "',to_date('" + datestr + "','yyyy-MM-dd hh24:mi:ss'),'DOING','" + proj_code + "','" + backup_type + "','"+ note + "','APP')";
	//out.println(sql+"<br>");
	i = db.execUpdate(sql);
	//out.println(i+"<br>");
	if(i==1){
