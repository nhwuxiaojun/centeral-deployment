...
class Datagrid  extends Dataview{
	...
	*/
	...
	public function makeSearchSql() {
		$sql="";
		//查询计算
		if(!isset($_GET['datagrid_action'])) return '';
		$action=$_GET['datagrid_action'];
		...
		foreach ($this->searchFields as $searchField => $searchName) {
			if(!isset($_GET[$searchField.'_exp']) || !isset($_GET[$searchField.'_value'])) continue;
			$exp=$_GET[$searchField.'_exp'];
			$value=strval($_GET[$searchField.'_value']);
	...
	public  function makeSql($makeCountSql=false) {
		...
		if(empty($sql)){
			if(empty($this->fields) || empty($this->tables)) die($GLOBALS['language']['error_empty_table_fields']);
		...
		//分页计算
		if(isset($_GET[$var_name.'_page'])){
			$page=intval($_GET[$var_name.'_page']);
			if($page<=0) $page=1;
			$this->page=$page;
	...
	public function setSearchHtml() {
		...
		$this->searchHtml='
			...
			<form action="" method="get" name="'.$var_name.'_search_form" id="'.$var_name.'_search_form">
			...
			<table  id="'.$var_name.'_form_table">
		           <tr>
			        <td  colspan="'.strval($numCos*3).'"  ><span  >'.$GLOBALS['language']['search_title'].'</span></td>
			      </tr> 
			      ';
		$num=0;
		for($i=0;$i<$numTR;$i++){
		...
		$this->searchHtml.='
		   ...
		   <tr >
			        <td colspan="'.strval($numCos*3).'">
			          <div align="center">
  					<input type="submit" name="Submit" value="'.$GLOBALS['language']['form_submit'].'">
 					 &nbsp;&nbsp;<input type="reset" name="Submit2" value="'.$GLOBALS['language']['form_reset'].'">
			          </div></td>
		          </tr> </table>	
	  <br>
   </form> 
	...
	function setTableFooterHtml() {
		...
		<tfoot id="'.$var_name.'_tfoot">
			<tr id="'.$var_name.'_tfoot_tr">
				<td id="'.$var_name.'_tfoot_td"   colspan="'.strval(count($this->fields)+2).'"   >'.$GLOBALS['language']['total'].'：'.$this->rowCount.' &nbsp;'.$this->getPagesInfo().'</td>
		 	 </tr>
	 	</tfoot>'."\n";
	}
	...
	public function addRow() {
		...
		if(empty($this->formTable)) return $GLOBALS['language']['error_noset_formTable']; 
	...
	public function updateRow() {
		...
		if(empty($_POST[$formPrimkey])){
			die( $GLOBALS['language']['error_noset_formPrimkey']);//"提交的主键字段{$formPrimkey}的值为空,操作不能完成!"
		}
	...
	public function deleteRow() {
		...
		$sql.=" WHERE $formPrimkey='".$_GET[$formPrimkey]."' limit 1";
		$flag=$this->db->query($sql);
		if($flag>0) return true;
		return false;
	} 
	...
	public  function makeFormHtml($flag="new") {
		...
		if($flag=="edit" || $flag=="view" || $flag=="copy" ){
			...
			if(!isset($_GET[$formPrimkey])){
				die( $GLOBALS['language']['error_noset_primkey']);
			}
			if($_GET[$formPrimkey]=="") die("{$formPrimkey},".$GLOBALS['language']['error_empty_GET_primkey']);
			$formPrimkeyValue=mysql_escape_string($_GET[$formPrimkey]);
		...
		$html='
			...
			<input type="hidden" name="var_name" value="'.$var_name.'" /> 
			';
		$title=$GLOBALS['language']['form_title_add'];
		if($flag=="edit"){
			$title=$GLOBALS['language']['form_title_edit'];
			$html.='<input type="hidden" name="'.$formPrimkey.'" value="'.$formPrimkeyValue.'" />';
		}
		if($flag=="view") $title=$GLOBALS['language']['form_title_view'];
		if($flag=="copy") $title=$GLOBALS['language']['form_title_copy'];
		$html.='
	...
	function display() {
		...
		if(isset($_POST['datagrid_action'])){
			...
			if($action=="new" || $action=="copy")  {
				if(!in_array("a",$roles)) return "对不起,您没有权限进行此操作!";
				$insertid=$this->addRow();
			}
			if($action=="edit") {
				if(!in_array("u",$roles)) return "对不起,您没有权限进行此操作!";
				$this->updateRow();
			}
		}else{
			//显示新增编辑
			if(isset($_GET['datagrid_action'])){
				$action=$_GET['datagrid_action'];
				if($action=="new")  {
					if(!in_array("a",$roles)) return "对不起,您没有权限进行此操作!";
				...
				if($action=="view") {
					if(!in_array("v",$roles)) return "对不起,您没有权限进行此操作!";
					return  $this->headerJS.$this->setViewFormHtml();
				}
				if($action=="delete")      {
					if(!in_array("d",$roles)) return "对不起,您没有权限进行此操作!";
					$this->deleteRow();
				}
			}
		}
	...
	public function makeHtml() {
		...
		$html.='<table summary=" table of Datagrid"  >';
	...
	function getPagesInfo($finally=true){
		...
		if (isset($_GET[$var_name.'_page'])) {		//设置 数
			$cur_page 			= intval($_GET[$var_name.'_page']);
	...
	public function getPageStr($pages,$page,$page_url_name="datagrid_page"){
		...
		if($pages>1){
			...
			if($page==1) $pre=1;
			$page_str= ' ' ;
			$page_str.= ' <a  href="'.$new_url."&".$page_url_name.'='.intval(1).'" >'.$GLOBALS['language']['page_first'].'</a>&nbsp;<a  href="'.$new_url."&".$page_url_name.'='.intval($pre).'"   >'.$GLOBALS['language']['page_pre'].'</a>&nbsp;
			 '; 
			if($pages>9){
				if($page<=3){
					for($i=1;$i<$page;$i++){
			...
			$page_str.= '<a href="'.$new_url."&".$page_url_name.'='.intval($next).'"    >'.$GLOBALS['language']['page_next'].'</a>
			&nbsp;<a href="'.$new_url."&".$page_url_name.'='.intval($pages).'"   >'.$GLOBALS['language']['page_end'].'</a>&nbsp;&nbsp;';
	...
	public function uploaderFILES($file){
		...
		if($file["error"]!=UPLOAD_ERR_OK) 				return array(false,$GLOBALS['language']['upload_error']);
		if($file["name"]=="")							return array(false,$GLOBALS['language']['upload_empty']);
		if($file["size"]>$max_file_size)			 	return array(false,$GLOBALS['language']['upload_maxsize']);
		$oldFilename = $file["name"];
		$filename = explode(".",$oldFilename);
		$filenameext = $filename[count($filename)-1];

		if(!in_array($filenameext,$file_types_array)) 	return array(false,$GLOBALS['language']['upload_valid']);
		$newFilename=md5($oldFilename)."_".time()."_".rand(10000,20000).".".$filenameext;
		if(!is_dir($upload_dir)) $this->createfolder($upload_dir);
		if(!is_dir($upload_dir)) 						return array(false,"".$upload_dir.$GLOBALS['language']['upload_dir_no_exist']);
		if(move_uploaded_file($file["tmp_name"], $this->relativeDir.$upload_dir.'/'.$newFilename)){
			return array(true, $upload_dir.$newFilename);
		}
		return array(false,"$oldFilename ".$GLOBALS['language']['upload_no_succ']);
