...
if("y".equals(request.getParameter("flag"))){
	if("".equals(request.getParameter("txt_ts_owner"))){
		if("".equals(request.getParameter("txt_submit_datetime"))){
			sql = "select * from a_pc_apply where status='DOING' order by apply_id desc";
		}else{
			sql = "select * from a_pc_apply where  status='DOING' and sysdate - apply_time <= " + request.getParameter("txt_submit_datetime") + " order by apply_id desc";
		}
	}else{
		if("".equals(request.getParameter("txt_submit_datetime"))){
			sql = "select * from a_pc_apply where  status='DOING'  and upper(ts_owner) like '%" + Tools.toGB(request.getParameter("txt_ts_owner").toString().trim()).toUpperCase() + "%' order by apply_id desc";
		}else{
			sql = "select * from a_pc_apply where  status='DOING'  and upper(ts_owner) like '%" + Tools.toGB(request.getParameter("txt_ts_owner").toString().trim()).toUpperCase() + "%' and sysdate - apply_time <= " + request.getParameter("txt_submit_datetime") + " order by apply_id desc";
		}
	}
}else{
	sql = "select * from (select * from a_pc_apply where  status='DOING'  order by apply_time desc) where rownum < 11";
}
//System.out.println(sql);
rs = db.execSQL(sql);
%>
<div align="center"><font color="#3333cc"><strong><br>
  <span class="s105">PC/VM答复</span></strong></font>
</div>
...
<table width="98%" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E" bordercolordark="#EFEFEF" align="center">
	<tr bgcolor="#D2D2D2">
	  ...
	  <%while(rs_user.next()){%>
	  	<%if("2".equals(rs_user.getString("valid"))){%>
			...
			<td align="center" width="10%"><font color="#3333cc"><strong>答复</strong></font></td>
	  	<%}%>
	  <%}%>
	</tr>
	<%while(rs.next()){%>
	<tr bgcolor="#FCFCFC">
	  ...
	  <%while(rs_user.next()){%>
	  	<%if("2".equals(rs_user.getString("valid"))){%>
	  		<td align="center">&nbsp;<a href="pc_del.jsp?apply_id=<%=rs.getInt("apply_id")%>"><font color="#3333cc">删除</font></a>&nbsp;</td>
			<td align="center">&nbsp;<a href="pc_reply.jsp?apply_id=<%=rs.getInt("apply_id")%>"><font color="#3333cc">答复</font></a>&nbsp;</td>
	  	<%}%>
	  <%}%>
    </tr>
	<%}%>
