...
//test
            ...
            $('#autotestbtn').click(function () {
                if (this.checked) {
                    $('#autotest').attr("value", "y");
                } else {
                    $('#autotest').attr("value", "n");
                }
            ...
            document.getElementById('deploy').onclick = function () {
                document.mainform.action = "ts_deploy_env_action.jsp?env_name=<%=request.getParameter("env_name")%>&env_id=<%=request.getParameter("env_id")%>&deploy_type=" + $(':radio[name="deploy_type"]:checked').val() + $('#deploy_input').val() + "&deploy_con=" + $(':radio[name="deploy_con"]:checked').val() + "&email=" + $('#email').val()+"&autotest=" + $('#autotest').val();
...
//    sql="select a.proj_name, a.dm from ts_proj a, ts_env b where a.proj_id=b.proj_id and b.env_id="+request.getParameter("env_id");
        sql = "select a.proj_name,d.inc_deploy_para, d.full_deploy_para, a.cm, a.dm,c.account from ts_proj a, ts_env b, ts_member c, ts_env_app d where b.env_id=d.env_id and  a.proj_name=b.proj_name and  a.proj_id=b.proj_id and a.owner1=c.no and b.env_id=" + request.getParameter("env_id");
        ...
        System.out.println(sql);
%>
<h2>Target Environment: <font color=red><%=request.getParameter("env_name")%>
</font></h2>
<hr>
<%
    ...
    while (rs.next() && sum < 1) {
        ...
        if ("null".equals(rs.getString("DM")) || "".equals(rs.getString("DM")) || rs.getString("DM") == null)
            dm = "no-dm";
        else {
            dm = rs.getString("DM");
            cm = rs.getString("CM");
...
<form id="mainform" name="mainform" method="POST" enctype="multipart/form-data"
      action="ts_deploy_env_action.jsp?env_name=<%=request.getParameter("env_name")%>&env_id=<%=request.getParameter("env_id")%>&deploy_type=inc_deploy">
    <div id="file_name">Package Upload:</div>
    <br>
    <input id="filename" type="file" name="file" size="60"
           onblur="$('#file_name').html('File Name: '+$('#filename').val()); "><br/>
    <br>
    ...
    <input id="deploy_batch" type="radio" value="batch" name="deploy_con">Batch</input>

    <br><br>
    App Deployment Type (install.sh parameters, available options highlighed below. <span
        style=background-color:yellow></span>):<br>
    <input id="inc_deploy" type="radio" value="inc_deploy" name="deploy_type" checked="checked">
    <%if (rs.getString("inc_deploy_para").equals("inc_deploy"))
        out.print("<span style=background-color:yellow><font color=red>"); %>Incremental Deployment
    (inc_deploy)</font></span></input>
    <br>
    <input id="increment_deploy" type="radio" value="increment_deploy"
           name="deploy_type" <% if (rs.getString("inc_deploy_para").equals("increment_deploy")) out.print("checked=checked"); %>>
    <%if (rs.getString("inc_deploy_para").equals("increment_deploy"))
        out.print("<span style=background-color:yellow><font color=red>"); %>Incremental Deployment
    (increment_deploy)</font></span></input>
    <br>
    <input id="full_deploy" type="radio" value="full_deploy" name="deploy_type">
    <%if (rs.getString("full_deploy_para").equals("full_deploy"))
        out.print("<span style=background-color:yellow><font color=red>"); %>Full Deployment
    (full_deploy)</font></span></input>
    <br>
    <input id="fully_deploy" type="radio" value="fully_deploy" name="deploy_type">
    <%if (rs.getString("full_deploy_para").equals("fully_deploy"))
        out.print("<span style=background-color:yellow><font color=red>"); %>Full Deployment
    (fully_deploy)</font></span></input>
    <br>
    <input id="upgrade_deploy" type="radio" value="upgrade"
           name="deploy_type" <% if (rs.getString("full_deploy_para").equals("upgrade")||rs.getString("inc_deploy_para").equals("upgrade")) out.print("checked=checked");%>>
    <%if (rs.getString("full_deploy_para").equals("upgrade") || rs.getString("inc_deploy_para").equals("upgrade"))
        out.print("<span style=background-color:yellow><font color=red>"); %>Upgrade Deployment
    (upgrade)</font></span></input>
    <br>
    <input id="cluster_inc_deploy" type="radio" value="cluster_inc_deploy"
           name="deploy_type" <% if (rs.getString("inc_deploy_para").equals("cluster_inc_deploy")) out.print("checked=checked"); %>>
    <%if (rs.getString("inc_deploy_para").equals("cluster_inc_deploy"))
        out.print("<span style=background-color:yellow><font color=red>"); %>Cluster Incremental Deployment
    (cluster_inc_deploy)</font></span></input>
    <br>
    <input id="other_type" type="radio" value="" name="deploy_type"></input><input id="deploy_input"
                                                                                   placeholder="customization option"
                                                                                   type="text"
                                                                                   value=""><%=rs.getString("inc_deploy_para")%>
    <br>

    <input id="autotestbtn" type="checkbox" checked="true" disabled="disabled">&nbsp;Auto Test</input>
    <input type="hidden" id="autotest" name="autotest" value="y">
...
<%
    //写文件
    String file_properties = "/usr/local/deploy/" + request.getParameter("env_name") + "/file.properties";
...
//f.setWritable(true);
    ...
    Runtime.getRuntime().exec("chmod 664 " + file_properties);
...
%>
<form action="ts_deploy_env.jsp" method="POST">
    <input id="updatebtn" type="checkbox">&nbsp;<font color=blue>Modify file.properties</font> (<%=filename%>)</input>
    &nbsp;&nbsp;
    <button type="submit" id="savebtn" disabled>Save</button>
    <br>
<textarea id="content" name="content" rows=3 cols=90 disabled=disabled>
<%
    String oneLine = "";
