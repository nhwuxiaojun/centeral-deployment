...
function add(){
	window.location="only_app_reply_add.jsp";
}
</script>
...
if("y".equals(request.getParameter("flag"))){
	if("".equals(request.getParameter("txt_new_env"))){
		if("".equals(request.getParameter("txt_submit_datetime"))){
			sql = "select * from a_supply where   apply_type='APP' order by id desc";
		}else{
			sql = "select * from a_supply where  apply_type='APP' and sysdate - apply_time <= " + request.getParameter("txt_submit_datetime") + " order by id desc";
		}
	}else{
		if("".equals(request.getParameter("txt_submit_datetime"))){
			sql = "select * from a_supply where  apply_type='APP'  and upper(new_env) like '%" + Tools.toGB(request.getParameter("txt_new_env").toString().trim()).toUpperCase() + "%' order by id desc";
		}else{
			sql = "select * from a_supply where  apply_type='APP' and  upper(new_env) like '%" + Tools.toGB(request.getParameter("txt_new_env").toString().trim()).toUpperCase() + "%' and sysdate - apply_time <= " + request.getParameter("txt_submit_datetime") + " order by id desc";
		}
	}
}else{
	sql = "select * from a_supply where  apply_type='APP'  and rownum<11 and (env_status='DOING' or db_status='DOING') order by id desc";
}
System.out.print(sql);
rs = db.execSQL(sql);
%>
<div align="center"><font color="#3333cc"><strong><br>
  <span class="s105">单应用环境答复列表</span></strong></font>
</div>
...
<table width="98%" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E" bordercolordark="#EFEFEF" align="center">
	...
	<tr bgcolor="#FCFCFC">
	  ...
	  <%while(rs_user.next()){%>
	  	<%if("2".equals(rs_user.getString("valid"))){%>
	  		<td align="center">&nbsp;<a href="only_app_db_apply_del.jsp?id=<%=rs.getInt("id")%>&flag=1"><font color="#3333cc">删除</font></a>&nbsp;</td>
	  	<%}%>
	  <%}%>
    </tr>
	<%}%>
