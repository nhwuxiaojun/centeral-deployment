...
ResultSet rs;
ResultSet rs_temp;
sql = "select * from a_proj_env_svr_tmp where env_id = " + request.getParameter("id");
rs = db.execSQL(sql);
%>
...
function return_back(){
	window.location = "env_info_query.jsp";
}
</script>
<script src="calendar2.js" language="javascript" type="text/javascript"></script>
<body><%@include file="includes/head.html" %>
<form name="form1" action="env_info_update_temp_finish.jsp" method="post">
<%while(rs.next()){%>
...
<table width="80%" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E" bordercolordark="#EFEFEF" align="center">
	...
	<tr>
	  ...
	  <%
	  sql_temp = "select distinct appsoft_ver from a_property where appsoft_ver is not null";
	  rs_temp = db.execSQL(sql_temp);
	  %>
	  <select name="APP_NAME">
	  	<%if(rs.getString("APP_NAME")==null){%>
			<option value=""><%=rs.getString("APP_NAME")%></option>
			<%while(rs_temp.next()){%>
