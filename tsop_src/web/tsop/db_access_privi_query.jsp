...
if("y".equals(request.getParameter("flag"))){
	if("".equals(request.getParameter("txt_db_ip"))){
		if("".equals(request.getParameter("col_value"))){
			sql = "select a.db_ip,b.login_db_sid,a.proj_env,a.db_user_type,b.login_db_user,b.login_ip,b.login_os_user from a_db_svr a right join a_login_control b on upper(a.db_user) = upper(b.login_db_user) and upper(a.db_sid) = upper(b.login_db_sid)";
		}else{
			sql = "select a.db_ip,b.login_db_sid,a.proj_env,a.db_user_type,b.login_db_user,b.login_ip,b.login_os_user from a_db_svr a right join a_login_control b on upper(a.db_user) = upper(b.login_db_user) and upper(a.db_sid) = upper(b.login_db_sid) where upper(" + request.getParameter("col_name").toString() + ") like '%" + request.getParameter("col_value").toString().trim().toUpperCase() + "%'";
		}
	}else{
		if("".equals(request.getParameter("col_value"))){
			sql = "select a.db_ip,b.login_db_sid,a.proj_env,a.db_user_type,b.login_db_user,b.login_ip,b.login_os_user from a_db_svr a right join a_login_control b on upper(a.db_user) = upper(b.login_db_user) and upper(a.db_sid) = upper(b.login_db_sid) where upper(a.db_ip) like '%" + request.getParameter("txt_db_ip").toString().trim().toUpperCase() + "%'";
		}else{
...
<%while(rs2.next()){%>
  <%if("2".equals(rs2.getString("valid"))){%>
	<tr bgcolor="#D2D2D2">
	  ...
	  <td width="10%" align="center" class="en"><font color="#3333cc"><strong>DB SID</strong></font></td>
	  <td width="10%" align="center" class="en"><font color="#3333cc"><strong>项目环境名</strong></font></td>
	  <td width="12%" align="center" class="en"><font color="#3333cc"><strong>用户类型</strong></font></td>
      <td width="15%" align="center" class="en"><font color="#3333cc"><strong>数据库用户</strong></font></td>
...
function del(){
  ...
  if (len==0){
    ...
    if (isdel!="0"){
      document.form2.action="db_access_privi_del.jsp";
