...
<center>
    <table align="center" cellpadding="0" cellspacing="0" border="0" width="98%">
        ...
        <form name="form1" action="ts_deploy_query.jsp" method="get" class="en">
            <input name="flag" value="y" type=hidden>
            <tr>
                <td align="left" class="en" width="20%">
                    &nbsp;<strong><font color="#3333cc" class="MenuBarItemSubmenu">Deployment Query</font></strong>
                    &nbsp;&nbsp;
...
//common_sql="select * from ts_env_app a, ts_db b, ts_env_db c, ts_env d, ts_proj e, ts_member f where c.db_ldap_name=b.LDAP_NAME(+) and a.env_id=c.env_id(+) and a.env_id=d.env_id(+) and d.proj_id=e.proj_id(+) and lower(c.DB_USER_TYPE)='owner' and e.owner1=f.no(+)";
                        common_sql = "select a.ENV_IP,a.ENV_ID,d.env_name,a.app_url,g.version,g.result,a.ENV_USR,a.ENV_PWD,a.APP_PORT,b.LDAP_NAME,c.DB_USER,c.DB_USER_PASSWD,f.account,g.start_time,g.end_time,g.deployer from ts_env_app a, ts_db b, ts_env_db c, ts_env d, ts_proj e, ts_member f, ts_deploy_log g where c.db_ldap_name=b.LDAP_NAME(+) and a.env_id=c.env_id(+) and a.env_id=d.env_id(+) and d.proj_id=e.proj_id(+) and lower(c.DB_USER_TYPE)='owner' and e.owner1=f.no(+) and a.env_id=g.env_id(+) and g.start_time is not null";
//common_sql="select * from ts_env_app a, ts_db b, ts_env_db c, ts_env d, ts_proj e, ts_member f where a.db_ldap_name=b.LDAP_NAME(+) and a.env_id=c.env_id(+) and a.env_id=d.env_id(+) and d.proj_id=e.proj_id(+) and lower(c.DB_USER_TYPE)='owner' and e.owner1=f.no(+)";
                        ...
                        if ("".equals(col_value1) || (col_value1 == null)) {
                            if (tmpproj_name == null || "".equals(tmpproj_name)) {
                                ...
                                sql = "select * from (" + common_sql + " and env_state='running' order by g.start_time desc ) where  rownum<18";
                                System.out.println("---sql1---");
                            } else {
                                // search by project name
                                sql = common_sql + " and env_state='running' and d.proj_id like '" + tmpproj_name + "' order by g.start_time desc";
                            ...
                            if (col_value1.indexOf("%") >= 0 || col_value1.trim() == "" || col_value1.trim().indexOf(" ") >= 0 || col_value1.length() < 2) {
                                //if (1==2) {
                                sql = common_sql + " and env_state='running' and rownum<18 order by g.start_time desc";
                                System.out.println("---sql3---");
                                ...
                                if ("ext_env_ip".equals(col_name1)) {
                                    ...
                                    sql = common_sql + " and env_state='running' and upper(" + col_name1.toString() + ") like '%" + col_value1.toString().trim().toUpperCase() + "%' order by g.start_time desc";
                                }
...
//		out.print("Search Condition: <font color=red><b>"+col_name1.toString()+"="+col_value1.toString().trim()+"</b><font>");
        ...
        <tr>
            <td align="center">
                <form name="form3" action="ts_deploy_query.jsp">
    ...
    <form name="form2">
        ...
        <table id="maintable" width="98%" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E"
            ...
            <tr bgcolor="#3333cc" height="24">
                ...
                <td align="center" class="en" width="6%"><font color="#ffffff"><b>Check Log</b></font></td>
                ...
                <td align="center" class="en" width="7%"><font color="#ffffff"><b>App Status</b></font></td>
            ...
            <td align=center><a href="javascript:"
                ...
                <a href="javascript:"
                   onClick="execSamba('\\\\172.17.1.7\\Application\\deploy\\<%=rs.getString("ENV_NAME")%>')">SMB</a>
            </td>
