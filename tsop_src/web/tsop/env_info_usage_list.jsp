	...
	if(request.getParameter("txt_env_state")==null||"".equals(request.getParameter("txt_env_state"))){
		if("".equals(request.getParameter("txt_env_ip"))){
			if("".equals(request.getParameter("col_value"))){
				sql="select * from a_proj_env_svr order by mac_ip,proj_env,env_ip";
			}else{
				sql="select * from a_proj_env_svr where upper(" + request.getParameter("col_name").toString() + ") like '%" + request.getParameter("col_value").toString().trim().toUpperCase() + "%' order by mac_ip,proj_env,env_ip";
			}
		}else{
			if("".equals(request.getParameter("col_value"))){
				sql="select * from a_proj_env_svr where upper(env_ip) like '%" + request.getParameter("txt_env_ip").toString().trim().toUpperCase() + "%' order by mac_ip,proj_env,env_ip";
			}else{
				sql="select * from a_proj_env_svr where upper(env_ip) like '%" + request.getParameter("txt_env_ip").toString().trim().toUpperCase() + "%' and upper(" + request.getParameter("col_name").toString() + ") like '%" + request.getParameter("col_value").toString().trim().toUpperCase() + "%' order by mac_ip,proj_env,env_ip";
			}
		}
	}else{
		if("".equals(request.getParameter("txt_env_ip"))){
			if("".equals(request.getParameter("col_value"))){
				...
				sql="select * from a_proj_env_svr where upper(" + request.getParameter("col_name").toString() + ") like '%" + request.getParameter("col_value").toString().trim().toUpperCase() + "%' and upper(env_state) = '" + request.getParameter("txt_env_state").toUpperCase().trim() + "' order by mac_ip,proj_env,env_ip";
			}
		}else{
			if("".equals(request.getParameter("col_value"))){
				sql="select * from a_proj_env_svr where upper(env_ip) like '%" + request.getParameter("txt_env_ip").toString().trim().toUpperCase() + "%' and upper(env_state) = '" + request.getParameter("txt_env_state").toUpperCase().trim() + "' order by mac_ip,proj_env,env_ip";
			}else{
				sql="select * from a_proj_env_svr where upper(env_ip) like '%" + request.getParameter("txt_env_ip").toString().trim().toUpperCase() + "%' and upper(" + request.getParameter("col_name").toString() + ") like '%" + request.getParameter("col_value").toString().trim().toUpperCase() + "%' and upper(env_state) = '" + request.getParameter("txt_env_state").toUpperCase().trim() + "' order by mac_ip,proj_env,env_ip";
			}
		}
	}
}else{
	sql="select * from a_proj_env_svr where rownum<11 order by mac_ip,proj_env,env_ip";
}
System.out.println(sql);
rs = db.execSQL(sql);
%>
<div align="center"><font color="#3333cc"><strong><br>
  <span class="s105">环境使用情况</span></strong></font>
</div>
...
<table width="98%" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E" bordercolordark="#EFEFEF" align="center">
	<tr bgcolor="#D2D2D2">
	  ...
	  <td align="center" class="en" width="13%"><font color="#3333cc"><strong>使用情况</strong></font></td>
	</tr>
	<%while(rs.next()){%>
	<tr bgcolor="#FCFCFC">
	  ...
	  <td align="center" bgcolor="#FCFCFC" class="en" nowrap>&nbsp;<a href=" http://172.16.29.59/awstats/awstats.pl?config=<%=rs.getString("ENV_IP")%>&framename=mainright#daysofmonth" target="_blank"><font color="#3333cc">查看</font></a>&nbsp;</td>
    </tr>
	<%}%>
</table>
