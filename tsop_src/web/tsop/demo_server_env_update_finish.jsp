...
sql = "update a_proj_env_svr set mac_ip = '" + mac_ip + "',env_ip = '" + env_ip + "', proj_env = '" + proj_env + "',web_inst_path = '" + web_inst_path + "',html_path = '" + html_path + "',app_name = '" + app_name + "',app_inst_path = '" + app_inst_path + "',app_port = '" + app_port + "',db_info = '" + db_info + "',db_usr = '" + db_usr + "',db_pwd = '" + db_pwd + "',env_usr = '" + env_usr + "',env_pwd = '" + env_pwd + "',con_usr = '" + con_usr + "',con_pwd = '" + con_pwd + "',ftp_usr = '" + ftp_usr + "',ftp_pwd = '" + ftp_pwd + "',proj_grp = '" + proj_grp + "',proj_dns = '" + proj_dns + "',creater = '" + creater + "',crt_date = to_date('" + crt_date + "','yyyy-mm-dd'),proposer = '" + proposer + "',env_state = '" + env_state + "',proj_code = " + proj_code + ",latest_update_date = to_date('" + datestr +"','yyyy-mm-dd hh24:mi:ss'),backup_dir = '" + backup_dir + "',note = '" + note + "' where env_id = " + env_id;
...
if(i==1){
	...
	message.setSubject("TABLE A_PROJ_ENV_SVR UPDATED");
	message.setDataHandler(new DataHandler(message,"text/html"));
	BodyPart mdp = new MimeBodyPart();
	content = creater + " update table a_proj_env_svr at " + datestr;
	...
	//发送邮件结束
	response.sendRedirect("demo_server_env_list.jsp?info=Update Succeed!");
}else{
	response.sendRedirect("demo_server_env_list.jsp?info=Update Failed!");
}
%>
