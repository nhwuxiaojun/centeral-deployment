...
//}
function add(){
	window.location="only_db_build_add.jsp";
}
</script>
...
if("y".equals(request.getParameter("flag"))){
	if("".equals(request.getParameter("txt_new_env"))){
		if("".equals(request.getParameter("txt_submit_datetime"))){
			sql = "select * from a_apply where  apply_type='DB' order by id desc";
		}else{
			sql = "select * from a_apply where  apply_type='DB'  and sysdate - apply_time <= " + request.getParameter("txt_submit_datetime") + " order by id desc";
		}
	}else{
		if("".equals(request.getParameter("txt_submit_datetime"))){
			sql = "select * from a_apply where  apply_type='DB' and  upper(new_env) like '%" + Tools.toGB(request.getParameter("txt_new_env").toString().trim()).toUpperCase() + "%' order by id desc";
		}else{
			sql = "select * from a_apply where  apply_type='DB'  and upper(new_env) like '%" + Tools.toGB(request.getParameter("txt_new_env").toString().trim()).toUpperCase() + "%' and sysdate - apply_time <= " + request.getParameter("txt_submit_datetime") + " order by id desc";
		}
	}
}else{
	sql = "select * from (select * from a_apply where  apply_type='DB'  order by apply_time desc) where rownum < 11";
}
//System.out.println(sql);
rs = db.execSQL(sql);
%>
<div align="center"><font color="#3333cc"><strong><br>
  <span class="s105">单数据库环境申请列表</span></strong></font>
</div>
...
<table width="98%" border="1" cellpadding="1" cellspacing="1" bordercolorlight="#9E9E9E" bordercolordark="#EFEFEF" align="center">
	<tr bgcolor="#D2D2D2">
	  <td align="center" width="30%"><font color="#3333cc"><strong>新环境名称</strong></font></td>
	  <td align="center" width="30%"><font color="#3333cc"><strong>申请提交时间</strong></font></td>
	  <td align="center" width="10%"><font color="#3333cc"><strong>状态</strong></font></td>
	...
	<tr bgcolor="#FCFCFC">
      ...
      <%}%>
	  <td align="center" >&nbsp;<a href="project_build_detail.jsp?id=<%=rs.getInt("id")%>" target="_blank"><font color="#3333cc">详细</font></a>&nbsp;</td>
	  <%sql_user = "select * from a_user where usr = '" + session.getAttribute("username") + "'";%>
	  <%rs_user = db.execSQL(sql_user);%>
	  <%while(rs_user.next()){%>
	  	<%if("2".equals(rs_user.getString("valid"))){%>
	  		<td align="center">&nbsp;<a href="only_app_db_build_del.jsp?id=<%=rs.getInt("id")%>&flag=2"><font color="#3333cc">删除</font></a>&nbsp;</td>
	  	<%}%>
	  <%}%>
    </tr>
	<%}%>
