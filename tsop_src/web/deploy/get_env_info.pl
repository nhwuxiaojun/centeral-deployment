...
while (@recs=$sth->fetchrow_array) {
	open (OUTPUT,">/usr/local/deploy/$recs[7]/env_info.sh") or die "cannot open file $recs[7]/env_info.sh for writing. ";
#	print "export HOST=$recs[0]\n";
        print OUTPUT "export HOST=$recs[0]\n";
        print OUTPUT "export LOGID=$recs[1]\n";
        print OUTPUT "export SSHPASS=$recs[2]\n";
        print OUTPUT "export APP_PATH=$recs[3]\n";
        print OUTPUT "export PATCH_PATH=\$APP_PATH/deploy\n";
        print OUTPUT "export DBUSER=$recs[4]\n";
        print OUTPUT "export DBPASS=$recs[5]\n";
        print OUTPUT "export DBLDAP=$recs[6]\n";
        print OUTPUT "export PROJNAME=$recs[7]\n";
        print OUTPUT "export TS=$recs[8]\n";
        print OUTPUT "export DM=$recs[9]\n";
        print OUTPUT "export ENV_ID=$recs[10]\n";
        print OUTPUT "export LOG_ID=$recs[11]\n";
        print OUTPUT "export CM=$recs[12]\n";
        print OUTPUT "export DBIP=$recs[13]\n";
        print OUTPUT "export DBSID=$recs[14]\n";
        print OUTPUT "export DBPORT=$recs[15]\n";
	print OUTPUT "export PRINT_IP=$recs[16]\n";
	print OUTPUT "export PRINT_USER=$recs[17]\n";
	print OUTPUT "export PRINT_PWD=$recs[18]\n";
	print OUTPUT "export PRINT_PATH=\"$recs[19]\"\n";
	exit 0;
} # end of while
