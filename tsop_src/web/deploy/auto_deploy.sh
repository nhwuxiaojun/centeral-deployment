...
cd ${PKG_PATH}/${ENV_NAME}
# deploy app files
if [ -d ${PKG_PATH}/${ENV_NAME}/$DT/EAR_FILE -a "all" = $DEPLOY_CON ] || [ -d ${PKG_PATH}/${ENV_NAME}/$DT/EAR_FILE -a "app" = $DEPLOY_CON ] || [ -f ${PKG_PATH}/${ENV_NAME}/$DT/upgrade_life_app.sh -a "all" = $DEPLOY_CON ] || [ -f ${PKG_PATH}/${ENV_NAME}/$DT/upgrade_life_app.sh -a "app" = $DEPLOY_CON ]
then
...
#  zip ${DT}_db_log.zip $LOGFILE `find ${DT} -mtime -1 -name 'upgrade*.log'`
  ...
  if [ $? -eq 0 ]; then 
        cat $LOGFILE | email -high-priority -n $MAILFROM -f $MAILFROM -s "[Failed] Deploy ${PKG_NAME} to ${PROJNAME} failed!" $MAILIST
    ...
    if [ $? -eq 0 ]; then
    	cat $LOGFILE | email -n $MAILFROM -f $MAILFROM -s "[WARNING] Deploy ${PKG_NAME} to ${PROJNAME} with warning!" $MAILIST 
        ...
        cat $LOGFILE | email -n $MAILFROM -f $MAILFROM -s "[Succeed] Deploy ${PKG_NAME} to ${PROJNAME} succeed!" $MAILIST
