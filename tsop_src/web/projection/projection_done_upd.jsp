...
String updpassword=request.getParameter("updpassword");
...
ResultSet rs2;
sql3 = "select * from a_projection_booking where id = " + id;
rs2 = db.execSQL(sql3);
while(rs2.next()){
	updpassword2 = rs2.getString("updpassword");
}
rs2.close();
...
if(flag1==0){
	...
	if(flag2==1){
		response.sendRedirect("projection_booking_view.jsp?msg=Your booking update successfully!");
	}else{
		response.sendRedirect("projection_booking_view.jsp?msg=Your booking update failure!");
	}
}else{
	response.sendRedirect("projection_booking_view.jsp?msg=Your booking update failure!(met conflicting)");
}
db.close();
rs1.close();
